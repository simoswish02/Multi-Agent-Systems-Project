...
Multi-drone cooperative search trained with **Independent / parameter-sharing
Dueling Double DQN**: a single shared Q-network drives every drone. This document
covers the module layout, the data flow of one episode, the observation space,
the neural network (branch `convnext_attn_net`), the context vector, and the
project invariants.
...
every drone shares one network, the agent count, vision radius, and comm range
are randomized per episode (**domain randomization**) and fed to the network as a
**context vector**, so a single policy generalizes across team sizes and sensor
ranges. Training follows a **curriculum** of phases; phases with `n_episodes: 0`
are skipped, so a run is staged by editing episode counts rather than code.
...
1. **Sample context.** `sample_ctx_params(dr_cfg)` draws `vision_radius`,
   `comm_range`, `n_agents` uniformly (inclusive `randint`) from the phase's DR
   ranges.
2. **Apply + reset.** `env.set_domain_params(**ctx_params)` makes the env's actual
   FOV / comm range / team size match the sampled context (coherent partial
   observability), then `env.reset()` → `generate_grid` builds a BFS-reachable
   map, all drones spawn at one random corner, each drone's vision updates its
   maps, `_communicate()` fuses drones within the sampled `comm_range`,
   observations are built. The team size for the episode is read back as
   `env.n_agents`.
3. **Per round, until done:**
   1. Build the context array once per episode with `ctx_to_numpy(ctx_params,
      ctx_norm)` → normalized `[vision, comm, n_agents]`. Position is no longer
      here — it rides in the `Own_Position` obs channel — so the context is
      constant within the episode and shared by every agent.
   2. Compute action masks `env._get_action_mask(i)`.
   3. `agent.select_actions_batch(obs, ctx_nps, masks)` — one shared forward for
      all greedy drones; ε-random for exploring drones (if **all** drones
      explore, no forward runs at all).
   4. For each drone `i`: `env.step_agent(i, action)` returns `(obs_i, reward,
      terminated, truncated, info)`. Inside: move/validate, revisit + collision +
      exploration + step/wall reward terms, map update, `_communicate()`, target
      check.
   5. Each per-agent step is fed to a per-agent `NStepBuffer`; once a window of
      `n_step` is complete it emits a transition `(s_t, a_t, R_t^{(n)}, s_{t+n},
      γ^n)` to the replay buffer (the bootstrap discount is `0` when the window
      hits a terminal — **truncation is not terminal**). At episode end the windows
      are flushed; with `shared_target_reward` the team terminal reward is applied
      first.
...
|-----|----------------------|----------|
| `global` | `GLOBAL_CHANNELS · gs² = 5 · 1024 = 5120` | `[Visited, Obstacle, Trajectory, Target, Own_Position]` |
...
| 4 | Other_Position | `{0,1}` (**local** only) — other drones within vision |
...
drone-centred, so it needs no own-position channel.
...
`OWN_POSITION_CHANNEL=4`) are defined at the top of the file and duplicated in
`env/grid_env.py` (Invariant 3).
...
```
input (B, 5, 32, 32)
  stem:   Conv3x3(5→96) + LayerNorm2d                      → (B, 96, 32, 32)
...
combined    = LayerNorm( cat[global_feat, local_feat, ctx] )   (1024+384+3 = 1411)
h  = fc1(1411→1536) → fc2(1536→768) → fc3(768→384)   [each: Linear+LayerNorm+GELU]
...
## 6. Context vector (`CTX_DIM = 3`)

`ctx_to_numpy(ctx_params, ctx_norm)` (in `training/train.py`) builds a `(3,)`
float32 vector:
...
All three components condition the FiLM layers and the attention context token,
and are concatenated into the final FC head. Because none of them changes within
an episode, the context is **constant per episode** and shared by every agent.

> Position used to be components 3–4 (`pos_row`, `pos_col`). It now travels as
> the `Own_Position` global observation channel (§4), from which `forward()`
...
3. **Channel-layout constants are duplicated** in `agents/networks.py` and
   `env/grid_env.py` (`GLOBAL_CHANNELS=5`, `LOCAL_CHANNELS=5`, global channel
   order `Visited / Obstacle / Trajectory / Target / Own_Position`; local channel
   4 is `Other_Position`). Intentionally no shared import — **keep the two copies
   in sync.**
4. **Context normalization derives from `domain_randomization` max values**
   (`build_ctx_norm`). Don't hardcode denominators elsewhere.
5. **`set_domain_params(...)` must precede every `reset()`** so the actual FOV /
   comm range / team size match what the context vector tells the network.
6. **Sequential execution.** No vectorized `env.step`; `env.step_agent(i, a)` per
   drone per round. Team size for an episode is read back as `env.n_agents`.
7. **Truncation is not termination.** Transitions store `terminated` only; running
   out of time at `max_steps · n_agents` must still bootstrap the next-state value.
8. **`shared_target_reward: true` requires `agent.n_step >= 2`** (the team
   terminal reward is credited through the still-pending n-step windows).
...
| `python main.py --mode eval --checkpoint PATH [--phase N] [--vision-radius/--comm-range/--n-agents]` | GUI evaluation episodes. |
| `python main.py --mode play` | Keyboard-controlled drone 0, others random. |
| `python main.py --mode simulate` | GUI config screen → run chosen episodes (spawn forced to top-left). |
| `python eval_checkpoints.py [...]` | Rank many checkpoints on identical fixed-seed instances. |
