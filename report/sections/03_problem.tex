...
division of the search space must be produced by the drones themselves ---
nothing in the environment assigns regions or roles --- and at any turn a live
drone may irreversibly break (Section~\ref{subsec:prob-fault}), leaving the
survivors to absorb the loss without any central entity telling them a teammate
is gone.
...
\citep{littman1994markov,oliehoek2016concise} --- the multi-agent decision
model reviewed in Section~\ref{subsec:bg-marl} --- with identical-interest
...
observation is a deterministic rendering of $K_i^t$
(Section~\ref{subsec:prob-obs}), so the process is Markovian in $s^t$ and partial
observability arises precisely from the gap between $K_i^t$ and the full state.
Episode-level parameters $\theta = (\rvis, \rcomm, \nagents, \rho, \pfault)$ are
drawn at the start of every episode and held fixed within it, a training-time
design choice discussed in Section~\ref{sec:training}.
...
never stretches the time budget. Sequential execution gives collisions and map updates an unambiguous semantics
--- there are no simultaneous-move conflicts to resolve --- and matches the
execution model of Section~\ref{sec:architecture}.

\paragraph{Transition.}
The transition kernel $P$ composes a stochastic fault event with a
...
state change is possible anymore. Faults are likewise truncations of the
broken drone's experience stream rather than terminal states; the learning
consequences of this distinction are worked out in
...
$\mathcal{C}_i$ record fault knowledge (Section~\ref{subsec:prob-fault}).
...
are the hand-crafted belief state of Section~\ref{subsec:bg-marl}, compressing
the interaction history into a spatial summary --- which is what allows a
memoryless policy to act reasonably under partial observability.
...
Section~\ref{sec:mas}. Fusion is \emph{anonymous} --- the max-merge keeps no
record of who contributed which cell, so shared knowledge detaches from its
producer; \emph{persistent} --- knowledge received in an encounter survives after
the drones part ways, being written into the receiver's own maps; and
\emph{selective} --- the trajectory map $\tau_i$ is excluded from fusion, so the
revisit-avoidance signal remains individual. Wrecks neither transmit nor
...
\begin{itemize}[leftmargin=1.5em]
  \item \emph{Time pressure and efficient coverage.} The constant step penalty
    ...
    teammate already swept earns nothing --- rewarding complementary rather than
    duplicated search.
...
\emph{time-to-find} (rounds elapsed at termination, i.e.\ agent-turns divided
by $\nagents$, over successful episodes), \emph{coverage} (fraction of free
cells discovered by the team),
