...
For completeness this appendix collects the figures referenced from the body but
not shown there. Figures~\ref{fig:app-vision} and~\ref{fig:app-maxsteps} give the
two one-factor sweeps summarized in Table~\ref{tab:generalization}, namely the
...
Section~\ref{sec:experiments}, since this bank samples the \emph{full}
randomization ranges (solo drones, $\rvis{=}1$, faults active) whereas the
evaluation there measures at the reference operating point. The
